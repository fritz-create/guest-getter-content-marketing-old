...
**On session start:** Read `00-START-HERE/SESSION-STATE.md` and root `session-notes.md`. If a phase is in progress, resume it. If not, check `CLIENT-INTAKE.md` - URL present = begin Phase 1 automatically.

**Cross-LLM handoff:** Keep `session-notes.md` updated with concise decisions, blockers, file changes, and next actions that future LLMs need. Do not duplicate full phase documents there. `SESSION-STATE.md` remains the formal phase tracker.
...
3. Build the Blog Tracker: select the strongest working title per topic, carry over tier, primary keyword if known, and intent fingerprint, rank all descending by priority score, save to `03-CALENDAR/backlog.csv`
...
02-TOPICAL-MAP/
  ...
  topical-map.csv        <- Phase 4B: topics + 3 working titles + optional primary keyword + scores per row
